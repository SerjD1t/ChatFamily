...
for (const mobile of [true,false]) test(`${mobile?'mobile':'desktop'}: retained chats and needs, family switching without replacing personal content`, {skip:!JSDOM}, async()=>{
 ...
 try{
  ...
  $('[name=needScope][value=personal]').click();await wait();
  ...
  $('[name=needScope][value=family]').click();await wait();
  assert.notEqual($('#shoppingForm'),privateForm);assert.equal($('#shoppingForm').elements.title.value,'');
  await switchFamily('f1');
  assert.ok($('#shoppingForm'));assert.equal($('[name=needScope]:checked').value,'family');
