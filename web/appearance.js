const choices={skin:['family','classic'],mode:['system','light','dark','contrast'],density:['normal','compact','spacious'],text:['normal','small','large']};
export const appearanceDefaults={skin:'family',mode:'system',density:'normal',text:'normal'};
...
const keyFor=id=>`familychat.appearance.v1.${encodeURIComponent(id)}`;
export function loadAppearance(storage,id,legacyMode) {
 try{return normalizeAppearance(JSON.parse(storage.getItem(keyFor(id))||'null'),legacyMode);}catch{return normalizeAppearance({},legacyMode);}
}
...
export function createAppearanceSettings({form,dialog,userID,preferences,storage=localStorage}) {
 ...
 function prepare(){
  ...
  ${select('interfaceDensity',t('Плотность','Density'),[['compact',t('Компактная','Compact')],['normal',t('Обычная','Normal')],['spacious',t('Просторная','Spacious')]])}
  ${select('interfaceTextSize',t('Размер текста','Text size'),[['small',t('Меньше','Smaller')],['normal',t('Обычный','Normal')],['large',t('Крупнее','Larger')]])}
