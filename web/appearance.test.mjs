...
test('appearance range keeps agreed text sizes, spacing factors and touch safeguards',()=>{
 ...
 for(const [size,value] of [['small','.8125rem'],['normal','1rem'],['large','1.3125rem']]) {
  assert.ok(css.includes(`html[data-text-size="${size}"] { --ui-font:${value}; }`));
 }
 assert.match(css,/data-density="compact"\] \{ --density-space:\.6;/);
 assert.match(css,/data-density="spacious"\] \{ --density-space:1\.4;/);
...
test('appearance is isolated between accounts/devices and failures are reported',()=>{
 ...
 assert.equal(loadAppearance(first,'b','light').skin,'family');assert.equal(loadAppearance(second,'a','light').skin,'family');
...
test('appearance preview cancels on close, resets only draft, saves without replacing history',{skip:!JSDOM},()=>{
 ...
 try{
  ...
  form.querySelector('#interfaceSkin').value='classic';form.dispatchEvent(new dom.window.Event('change',{bubbles:true}));
  assert.equal(document.documentElement.dataset.skin,'classic');assert.equal(loadAppearance(store,'a','dark').skin,'family');
  dialog.dispatchEvent(new dom.window.Event('close'));assert.equal(document.documentElement.dataset.skin,'family');
  ...
  form.querySelector('[data-appearance-reset]').click();assert.equal(document.documentElement.dataset.skin,'family');assert.equal(loadAppearance(store,'a','dark').skin,'classic');
